...
class TurtleControlNode(Node):
    ...
    def goalPoint_callback(self, msg):
        ...
        goalPoint = msg.data
        if goalPoint < len(self.positions):
            self.x_goal, self.y_goal = self.positions[goalPoint]
            self.goal_received = True
            self.get_logger().info(f'Objetivo atualizado para o ponto {goalPoint}: x_goal={self.x_goal}, y_goal={self.y_goal}')
